...
describe('non-régression — les URLs de sorts existantes ne changent pas', () => {
  ...
  it.each(URLS.map((url, i) => [i, url] as const))('URL #%d — EtatUrl inchangé', (_i, url) => {
    ...
    // Round-trips to the same canonical query string as before this step, and
    // carries no `dons` key — `lireEtat` was not modified to populate it.
    expect(ecrireEtat(etat)).toBeInstanceOf(URLSearchParams)
    expect(etat.dons).toBeUndefined()
    // Sanity: still resolves against the real fixture exactly as `lireEtat`
    // always has (any of its own existing keys, spot-checked).
    expect(typeof etat.classe === 'string' || etat.classe === null).toBe(true)
  })
