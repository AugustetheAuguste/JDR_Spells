...
import type { CatalogueDons } from './moteur'
import type { DonConditions, Personnage, ResultatEligibilite, TablesMoteur } from './types'
...
export function calculerVagues(
  ...
  const acquis = new Set(perso.dons_connus ?? [])
  ...
  for (let vague = 1; vague <= slots; vague += 1) {
    ...
    for (const [nom, don] of catalogue) {
      if (vagueDe.has(nom) || acquis.has(nom)) continue
      const resultat = evaluerDon(nom, don, courant, tablesDeVague)
...
// `calculerVagues` needs the eligibility tables threaded through to
// `evaluerDon` on every iteration; captured via a small indirection so the
// exported signature stays free of a `tables` positional the plan's
// pseudo-code doesn't show. Set once per call by `calculerVaguesAvecTables`.
let tablesDeVague: TablesMoteur

/** Wrapper that binds `tables` before delegating to `calculerVagues` — kept
 * separate so `calculerVagues` itself matches the plan's pseudo-code
 * signature exactly (`catalogue, perso, slots`, no `tables`). */
export function calculerVaguesAvecTables(
  perso: Personnage,
  catalogue: CatalogueDons,
  slots: number,
  tables: TablesMoteur,
): { readonly vagueDe: ReadonlyMap<string, number>; readonly resultatDe: ReadonlyMap<string, ResultatEligibilite> } {
  tablesDeVague = tables
  return calculerVagues(perso, catalogue, slots)
}
